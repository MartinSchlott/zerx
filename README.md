...
**Strict-by-default objects.** Unknown properties return a `ZerxError` with code `unknown_property` — a **security boundary**, not a style choice: it rejects typos and unexpected/injected fields at the edge. Relax per schema with `.passthrough()` (preserve unknowns) or `.strip()` (silently drop), or per import with `ImportOptions.strip_unknown` (§ below). Schema-level utilities (`omit`, `omit_read_only`, `omit_write_only`, `partial`, `extend`) and runtime-level ones (`strip_only`, `strip_read_only`, `strip_write_only`) coexist — the runtime layer filters input *before* the mode check, so you can keep strict mode and still drop a known set of keys.
...
**Policy-driven JSON Schema import.** `from_json_schema_with(&schema, &opts)` runs a composable pre-parse `SchemaTransform` pass over the input and a post-parse `TypeTransform` pass over the resulting types. The built-in `sql` policy (PostgreSQL-focused) maps `int64 → string`, `jsonb → zerx::json()`, `bytea → zerx::buffer()`, normalizes `anyOf` of `T | null` to `T.nullable()`, and applies SQL format mappings (e.g. `timestamp* → date-time`). Register your own with `zerx::register_policy(name, …)`. `ImportOptions.strip_unknown` (`false` by default) is an orthogonal caller-side disposition: `true` silently drops properties every imported object node would otherwise reject, recursively, everywhere `additionalProperties` is `false` or absent — the self-healing counterpart to strict-by-default, useful when a persisted config file carries a stale key from a schema that has since dropped a field.

**Bidirectional JSON Schema.** `to_json_schema` and `from_json_schema` are built for roundtrip stability. `$defs`/`$ref` survive, recursive structures survive (lazy placeholders + memoization), format markers survive. `oneOf` imports as a union with `x-oneOf` metadata. `allOf` and `not` raise clear errors instead of being silently dropped. `additionalProperties` handles all four input shapes (`true` / `false` / absent / schema object — the last treated as passthrough); `false` and absent import as strip mode instead of strict when the caller opts into `ImportOptions.strip_unknown` (a caller-side disposition, not carried by the document itself). Discriminated unions use Draft 2020-12 `discriminator` and reconstruct correctly even nested inside arrays.
