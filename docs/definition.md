...
- Zerx exists to validate real-world data in Rust at runtime — including data that is not JSON-clean (binary buffers, Lua values from sibling runtimes, PostgreSQL JSONBs) — against schemas assembled programmatically as runtime values.
...
- The product covers validation of non-JSON-clean values: binary buffers (with MIME) natively, and Lua values via schema-directed validation under an optional feature.
...
- `Non-JSON-clean validation` — buffers and Lua data values validate without first being made JSON-clean.
...
- `ZerxValue` — a validated dynamic value spanning serde's full data model.
...
- The product is successful when non-JSON data is first-class: buffers (with MIME) validate and roundtrip through format markers, and Lua data values validate via schema-directed disambiguation, without first being made `serde_json`-clean.
...
- The product is successful when schemas hand off to LLMs/tool-use as JSON Schema and their output, including Lua data from sibling runtimes, validates.
